	...
	*/
	...
	var validator = $( "#formId" ).validate({
		...
		rules: {
			...
			plot_flat_no: {
				required: true, pattern: "^[a-zA-Z\\d,\\-\/]+$", maxlength: 100
			},
			road_street: {
			...
			//dept_area_id: {required: true, min: 1},
			work_district_id: {
				required: true, min: 1
			},
			...
			appmade_email: {
				required: false,email: true, maxlength: 55,customemail: true
			},
			appmade_mob_num: {
				required: true, digits: true, minlength: 10, pattern: "^[6-9][0-9]{9}$" //"^[6-9][0-9]{9}$"
			},
			...
			payment_challan_no: {
				required: {
					depends: function(element){
						var paymentType = parseInt($('#payment_mode').val());
			...
			payment_in_favour: {
				required: true, pattern: "^[a-zA-Z\\s]+$", maxlength: 25
			},
			payment_date: {
				required: true,
				pattern: "^[\\d\/]+$",
		...
		messages: {
			...
			plot_flat_no: {
				required: "This field is required!",
				pattern: "Alphabets,Numbers, - and / are allowed!",
		...
		submitHandler: function(form) {
			...
			var submitVal = $("#submitValId").val();
			var confirmMsg = "";
			if(submitVal=="SubmitPay"){
				confirmMsg = "Are you sure you want to Submit?";
			}else{
				confirmMsg = "Are you sure you want to Update?";
			}
	...
	$("input[id*=document_Description_Num_]").each(function(){
		$(this).rules("add", {
			required: function(element) {
				return $(element).siblings('input[id*=is_label_mandatory_]').val() === "true";
			},
			messages: {
				required: "This field is required!",
			}
